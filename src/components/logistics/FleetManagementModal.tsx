...
export function FleetManagementModal({
      ...
      if (payload.ok) {
        showToast(
          payload.reassignedOrders
            ? `Veicolo eliminato. ${payload.reassignedOrders} ${payload.reassignedOrders === 1 ? "ordine spostato" : tr("ordini spostati")} in Non assegnati.`
            : tr("Veicolo eliminato."),
          "success"
        );
        refresh();
      } else {
  ...
  async function move(vehicleId: string, direction: -1 | 1) {
    ...
    try {
      ...
      else showToast(tr("Riordino non salvato."), "error");
    } catch {
      showToast(tr("Errore di rete."), "error");
    } finally {
      setBusyId(null);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 p-4" role="dialog" aria-modal="true">
      <div className="flex max-h-[85vh] w-full max-w-md flex-col rounded-2xl bg-white shadow-modal">
        <div className="flex items-center justify-between border-b border-ink/10 px-5 py-4">
          <h2 className="text-base font-bold text-ink">{tr("Veicoli")}</h2>
          <button
            type="button"
            onClick={onClose}
            aria-label="Chiudi"
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3">
          {ordered.length === 0 && <p className="py-6 text-center text-sm text-ink-soft">{tr("Nessun veicolo ancora.")}</p>}
                  ...
                  {renamingId === vehicle.id ? (
                    ...
                    <div className="rounded-xl bg-state-warning-soft p-3">
                      {count > 0 ? (
                        <>
                          <p className="text-sm font-semibold text-ink">
                            {vehicle.name} ha {count} {count === 1 ? "ordine assegnato" : tr("ordini assegnati")}
                          </p>
                          <p className="mt-1 text-xs text-ink-soft">
                            Se elimini questo veicolo, {count === 1 ? "l'ordine verrà spostato" : tr("tutti gli ordini verranno spostati")} in
                            Non assegnati.
                          </p>
                        </>
                      ) : (
                      ...
                      <div className="mt-2.5 flex gap-2">
                        ...
                        >
                          {busy ? "Rimozione…" : count > 0 ? "Sposta gli ordini ed elimina il veicolo" : tr("Elimina")}
                        </button>
                      </div>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="text-xs text-ink-soft">
                          ...
                          {count} {count === 1 ? "ordine" : tr("ordini")}
                        </div>
                      </div>
          ...
          {addingOpen ? (
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder={tr("Targa (opzionale)")}
                className="h-10 w-full rounded-lg border border-ink/15 px-3 text-sm text-ink outline-none focus:border-accent"
              />
