...
export function CustomerLoginForm() {
  ...
  async function submit(e:React.FormEvent){
    ...
    try {
      ...
      if(!r.ok||!p.ok){setError(p.code==="CUSTOMER_ACCOUNT_NOT_LINKED"?"Account non abilitato al portale.":"Email o password non validi.");return;}
      router.replace("/account"); router.refresh();
    } catch { setError("Accesso non disponibile. Riprova."); } finally {setBusy(false);}
  }
  return <form onSubmit={submit} className="mt-6 space-y-4">
    ...
    <Button type="submit" size="lg" disabled={busy} className="w-full">{busy?"Accesso…":"Accedi"}</Button>
  </form>;
}
