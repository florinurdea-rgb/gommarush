...
**FINDING B — `/cauta-cauciuc` was a dead public feature. RESOLVED: removed entirely.** It was customer-facing, linked from the homepage, rate-limited as a public endpoint, and returned "Căutarea automată nu este configurată" for every single scan, because `AnthropicTyreLookup` was its only provider. Per the operator's decision, the whole feature was deleted rather than ported: `app/cauta-cauciuc/`, `app/api/tyre-lookup/`, `src/lib/tyre-lookup/` (5 files), `src/components/tyre-lookup/TyreLookupScanner.tsx`, both test files, the homepage entry-point button, `isBarcodeLookupRateLimited()` and its `BARCODE_LOOKUP_RATE_LIMIT_*` env vars. Verified afterward: typecheck clean, 197/197 tests pass, production build emits 36 routes with no `cauta-cauciuc` or `tyre-lookup` entry.

> Why porting was the worse option: `anthropic-lookup.ts` used the Messages API's **native `web_search_20250305` server-side tool** to identify a tyre from a barcode against live web sources, with citation extraction feeding an "only trust an identification backed by ≥1 source" honesty rule. That has no drop-in OpenAI equivalent — a port meant re-architecting around a different search mechanism, not swapping a base URL. Flow B's document analyzer, by contrast, would port straightforwardly (§0 FINDING A remains open on that basis).
