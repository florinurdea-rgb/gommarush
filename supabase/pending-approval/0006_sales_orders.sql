...
begin
  ...
  insert into public.sales_order_items (
    ...
    pricing_status, price_observed_at
  )
  select
    v_order_id,
...
begin
  if exists (select 1 from pg_roles where rolname = 'anon') then
    execute 'revoke all on function public.create_portal_sales_order(uuid, uuid, text, jsonb, jsonb, jsonb, text, text, text, text, bigint, bigint, bigint, bigint, text, jsonb) from anon';
  end if;
  if exists (select 1 from pg_roles where rolname = 'authenticated') then
    execute 'revoke all on function public.create_portal_sales_order(uuid, uuid, text, jsonb, jsonb, jsonb, text, text, text, text, bigint, bigint, bigint, bigint, text, jsonb) from authenticated';
  end if;
end $$;
